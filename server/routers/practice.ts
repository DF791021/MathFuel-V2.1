...
export const practiceRouter = router({
  ...
  // Get adaptive skill recommendations for a student
  // Uses mastery levels and prerequisite chains to surface the most useful next skills.
  getRecommendedSkills: protectedProcedure
    .input(z.object({
      gradeLevel: z.number().int().min(1).max(12).default(1),
    ...
    .query(async ({ ctx, input }) => {
      ...
      const masteryRecords = await db.getStudentMastery(ctx.user.id);
      ...
      const scored = skills.map(skill => {
        ...
        if (mastery?.masteryLevel === "mastered") {
          return { skill, score: 0, reason: "Already mastered!" };
        }

        // Prerequisite must be mastered before starting this skill
        ...
        if (!prereqMet) {
          return { skill, score: 0, reason: "Complete prerequisite first" };
        }

        // "close" skills — one push away from mastery
        if (mastery?.masteryLevel === "close") {
          return {
            skill,
            score: 90,
            reason: `Almost mastered! You're at ${mastery.masteryScore}% 🎯`,
          };
        }
        ...
        // Not started yet but prerequisites are met
        return { skill, score: 35, reason: "New skill to explore! ✨" };
      });
      ...
      const recommendations = scored
        ...
        .map(s => ({
          ...
          masteryScore: masteryMap.get(s.skill.id)?.masteryScore ?? 0,
          masteryLevel: (masteryMap.get(s.skill.id)?.masteryLevel ?? "not_started") as
            "not_started" | "practicing" | "close" | "mastered",
          reason: s.reason,
        }));
